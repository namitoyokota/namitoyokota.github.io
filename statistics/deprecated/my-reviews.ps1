...
foreach ($repo in $repos.value) {
    ...
    :inner
    do {
        $PrEndpoint = "https://dev.azure.com/venminder/_apis/git/repositories/$($repo.id)/pullrequests?api-version=7.0&searchCriteria.status=completed&searchCriteria.reviewerId=$($uuid)&`$top=$($take)&`$skip=$($skip)"
        $skip += $take
...
write-output "`nTotal PRs Reviewed: $totalPrCount`n";
