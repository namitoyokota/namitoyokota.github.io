# This script retrieves and outputs statistics on Azure DevOps pull requests that I've been requested to review.
...
async def get_total_pull_requests(session, organization, repository_id, creator_id):
    endpoint = f"https://dev.azure.com/{organization}/_apis/git/repositories/{repository_id}/pullrequests?api-version=7.0&searchCriteria.status=completed&searchCriteria.reviewerId={creator_id}&$top=1"
...
async def get_next_pull_requests(session, organization, repository_id, creator_id, skip):
    endpoint = f"https://dev.azure.com/{organization}/_apis/git/repositories/{repository_id}/pullrequests?api-version=7.0&searchCriteria.status=completed&searchCriteria.reviewerId={creator_id}&$top={take}&$skip={skip}"
    response = await fetch(session, endpoint)
    return response.get("value", [])
...
async def process_repository(session, repo, creator_id, author, progress_bar):
    ...
    while True:
        ...
        for pr in pull_requests:
            ...
            repo_stats_by_year[year]["pull_requests_requested"] += 1
            if any(reviewer["vote"] == 10 for reviewer in pr["reviewers"] if reviewer["id"] == creator_id):
                repo_stats_by_year[year]["pull_requests_reviewed"] += 1
            ...
            for thread in threads:
                ...
                for comment in thread["comments"]:
                    if comment["author"]["id"] == creator_id:
                        repo_stats_by_year[year]["comments_given"] += 1
    ...
    return {year: stats for year, stats in repo_stats_by_year.items() if stats["pull_requests_requested"] > 0}
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # Export JSON files
        output_dir = os.path.join(os.path.dirname(__file__), "reviewed_prs")
        ...
        for repo_stats_by_year in repo_stats_by_year_list:
            for year, repo_stats in repo_stats_by_year.items():
                ...
                if repo_stats["pull_requests_requested"] > 0:
                    repo_stats["average_comments_received"] = repo_stats["comments_received"] / repo_stats["pull_requests_requested"]
                    repo_stats["average_comments_given"] = repo_stats["comments_given"] / repo_stats["pull_requests_requested"]
                    repo_stats["average_turnaround_hours"] = repo_stats["total_turnaround_hours"] / repo_stats["pull_requests_requested"]
                else:
                    repo_stats["average_comments_received"] = 0
                    repo_stats["average_comments_given"] = 0
        ...
        for year, total_stats in total_stats_by_year.items():
            year_dir = os.path.join(output_dir, str(year))
            if total_stats["pull_requests_requested"] > 0:
                total_stats["average_comments_received"] = total_stats["comments_received"] / total_stats["pull_requests_requested"]
                total_stats["average_comments_given"] = total_stats["comments_given"] / total_stats["pull_requests_requested"]
                total_stats["average_turnaround_hours"] = total_stats["total_turnaround_hours"] / total_stats["pull_requests_requested"]
            else:
                total_stats["average_comments_received"] = 0
                total_stats["average_comments_given"] = 0
        ...
        for repo_name, stats in total_stats_overall.items():
            if stats["pull_requests_requested"] > 0:
                stats["average_comments_received"] = stats["comments_received"] / stats["pull_requests_requested"]
                stats["average_comments_given"] = stats["comments_given"] / stats["pull_requests_requested"]
                stats["average_turnaround_hours"] = stats["total_turnaround_hours"] / stats["pull_requests_requested"]
            else:
                stats["average_comments_received"] = 0
                stats["average_comments_given"] = 0
        ...
        if overall_total_stats["pull_requests_requested"] > 0:
            overall_total_stats["average_comments_received"] = overall_total_stats["comments_received"] / overall_total_stats["pull_requests_requested"]
            overall_total_stats["average_comments_given"] = overall_total_stats["comments_given"] / overall_total_stats["pull_requests_requested"]
            overall_total_stats["average_turnaround_hours"] = overall_total_stats["total_turnaround_hours"] / overall_total_stats["pull_requests_requested"]
        else:
            overall_total_stats["average_comments_received"] = 0
            overall_total_stats["average_comments_given"] = 0
